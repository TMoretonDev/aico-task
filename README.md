...
An Nx monorepo containing the API, UI and shared-types package for the AICO take home task. The project is a simple CRUD API with a table component in the frontend to visualise the data.
...
E2E tests for the API live alongside the code they exercise (`apps/aico-task-api/src/**/*.e2e-spec.ts`) and run against a disposable Postgres instance defined in `docker-compose.test.yml`. The test DB uses `tmpfs` so data never touches disk — every run gets a fresh container on port `5434`, separate from the dev DB on `5433`.
...
- **No frontend tests**: Though I have backend API e2e tests that check for parameter validation and successful / failed CRUD operations, the frontend is currently untested due to it's simplicity and lack of requirement for integration/e2e tests. There's no complex hook logic to warrant unit tests and all components come directly from ShadCN which also eliminates the need for component level testing. That being said, if the project were to grow and the frontend gained increased complexity and areas of integration, there would be a need for frontend tests.
